# Module 09 - ClickHouse

## Duration

Approximately 1 hour.

## Purpose

This module explains why ClickHouse fits analytical telemetry workloads and how schema, ingestion and retention decisions affect observability performance.

## Main topics

- Columnar storage for telemetry.
- Schema design and order keys.
- Batch ingestion and retention.
- SQL investigation patterns.

## Training assets

- [Module narrative](module.md)
- [Exercise - Trace span table design](exercise.md)
- [Lab - ClickHouse observability query workbook](../../labs/module-09-clickhouse-query-workbook.md)
- [Quiz - Review questions and answers](quiz.md)
- [Official references](references.md)
- [Concept map](images/concept-map.svg)
- [ClickHouse telemetry workflow](images/workflow.svg)
- [Lab practice](images/lab-practice.svg)
- [Operations view](images/operations-view.svg)

## Status

Draft available.

## Next module

Module 10 - Grafana.
